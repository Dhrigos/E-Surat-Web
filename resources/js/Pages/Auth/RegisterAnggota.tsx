...
import { User, Mail, Lock, Phone, Shield, ArrowRight, Check, X, Eye, EyeOff, AlertCircle } from 'lucide-react';
...
export default function RegisterAnggota({ settings = {} }: { settings?: Record<string, any> }) {
    ...
    const isRegistrationOpen = React.useMemo(() => {
        if (!settings) return true; // Default to open if no settings passed (should verify with backend)
    ...
    if (!isRegistrationOpen) {
        return (
            <div className="fixed inset-0 bg-black overflow-hidden font-sans flex flex-col items-center justify-center">
                <Head title="Pendaftaran Tutup" />
                <Background />

                <div className="relative z-10 p-6 w-full max-w-md">
                    <Card className="bg-[#252525]/95 backdrop-blur-xl border-2 border-white/20 text-white shadow-2xl">
                        <CardHeader className="text-center">
                            <div className="flex justify-center mb-4">
                                <AlertCircle className="w-16 h-16 text-[#AC0021]" />
                            </div>
                            <CardTitle className="text-2xl font-black text-[#AC0021] mb-2">
                                Pendaftaran Ditutup
                            </CardTitle>
                            <CardDescription className="text-gray-300 text-base">
                                Mohon maaf, pendaftaran Komponen Cadangan saat ini sedang tidak menerima anggota baru.
                            </CardDescription>
                        </CardHeader>
                        <CardContent className="text-center space-y-4">
                            {settings.registration_start_date && (
                                <div className="text-sm bg-white/5 p-3 rounded-lg border border-white/10">
                                    <span className="text-gray-400">Jadwal Pendaftaran:</span>
                                    <div className="font-semibold text-white mt-1">
                                        {format(new Date(settings.registration_start_date), 'dd MMMM yyyy')} - {settings.registration_end_date ? format(new Date(settings.registration_end_date), 'dd MMMM yyyy') : 'Selesai'}
                                    </div>
                                </div>
                            )}

                            <Link href={route('login')}>
                                <Button className="w-full bg-[#AC0021] hover:bg-[#AC0021]/90 font-bold">
                                    Kembali ke Login
                                </Button>
                            </Link>
                        </CardContent>
