...
export default function CompleteProfile({ auth, jabatans, jabatanRoles = [], golongans = [], pangkats = [], rejectionReason, existingFiles }: Props) {
    ...
    useEffect(() => {
        ...
        // Set preview for existing KTP from eKYC
        if (existingFiles?.scan_ktp) {
            const isPdf = existingFiles.scan_ktp.toLowerCase().endsWith('.pdf');
            setPreviews(prev => ({ ...prev, scan_ktp: isPdf ? 'PDF_EXISTING' : existingFiles.scan_ktp! }));
        }
    }, [existingFiles]);
                            ...
                            <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-6 border-t border-white/10">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        >
                                            {previews.scan_sk ? (
                                                previews.scan_sk.toLowerCase().endsWith('.pdf') || previews.scan_sk.startsWith('blob:') && data.scan_sk?.type === 'application/pdf' ? (
                                                    <div className="w-full h-full relative group">
                                                        <iframe
                                                ...
                                                ) : previews.scan_sk === 'DOC_FILE' ? (
                                                    <div className="flex flex-col items-center justify-center text-[#FEFCF8] h-full">
                                                        <FileText className="w-12 h-12 mb-2 text-blue-400" />
                                                        <span className="text-sm font-medium">{data.scan_sk?.name || 'Dokumen'}</span>
                                                        <span className="text-xs text-gray-400 mt-1">
                                                            {data.scan_sk?.size ? `${(data.scan_sk.size / 1024).toFixed(1)} KB` : ''}
                                                        </span>
                                                        <div className="mt-3 text-xs bg-black/50 px-2 py-1 rounded flex items-center gap-1 cursor-pointer hover:bg-black/70" onClick={(e) => { e.stopPropagation(); skInputRef.current?.click(); }}>
                                                            <FileText className="w-3 h-3" /> Ganti File
                                                        </div>
                                                    </div>
                                                ) : (
