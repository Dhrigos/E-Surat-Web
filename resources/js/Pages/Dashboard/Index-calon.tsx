...
import { Activity, Mail, Send, Clock, FileText, CheckCircle2, XCircle, Download, MapPin, Building2, Anchor, Plane, Shield, Users } from 'lucide-react';
...
const IndonesiaMap = lazy(() => import('@/components/IndonesiaMap-calon'));
...
interface DashboardProps {
    ...
    provinceDetails?: Record<string, { name: string; L: number; P: number; total: number }[]>;
}

export default function Dashboard({ stats, chartData, activities, usersByProvince, totalUsers, adminStats, matraCounts, provinceDetails }: DashboardProps) {
    const { auth } = usePage<SharedData>().props;
    const isSuperAdmin = auth.user.roles?.some(r => r.name === 'super-admin') ?? false;
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [selectedProvince, setSelectedProvince] = useState('');
    const [selectedProvinceData, setSelectedProvinceData] = useState<{ name: string; L: number; P: number; total: number }[]>([]);

    const handleProvinceClick = (provinceName: string, provinceCode: string) => {
        setSelectedProvince(provinceName);

        // Use provinceCode directly from map click event
        if (provinceDetails && provinceDetails[provinceCode]) {
            setSelectedProvinceData(provinceDetails[provinceCode]);
        } else {
    ...
    return (
        <AppLayout className="min-h-full">
            ...
            {/* Calon Anggota View */}
            {(auth.user as any).member_type === 'calon_anggota' ? (
                <div className="flex flex-col gap-6 p-4 md:p-10 min-h-[80vh] items-center justify-center text-center">
                    <div className="bg-[#262626] border border-white/5 rounded-2xl p-8 md:p-12 max-w-2xl w-full shadow-2xl relative overflow-hidden group">
                        {/* Decorative Background Elements */}
                        <div className="absolute top-0 right-0 -mt-10 -mr-10 w-64 h-64 bg-red-500/10 rounded-full blur-3xl group-hover:bg-red-500/15 transition-all duration-500"></div>
                        <div className="absolute bottom-0 left-0 -mb-10 -ml-10 w-40 h-40 bg-blue-500/10 rounded-full blur-2xl group-hover:bg-blue-500/15 transition-all duration-500"></div>

                        <div className="relative z-10 flex flex-col items-center">
                            <div className="h-24 w-24 bg-white/5 rounded-full flex items-center justify-center mb-6 animate-pulse">
                                <img src="/images/BADAN-CADANGAN-NASIONAL.png" alt="Logo" className="h-16 object-contain" />
                            </div>

                            <h2 className="text-3xl md:text-4xl font-bold tracking-tight text-white mb-4">
                                Selamat Datang, <span className="text-[#AC0021]">{auth.user.name}</span>!
                            </h2>

                            <p className="text-lg text-gray-400 max-w-lg mx-auto mb-8">
                                Terima kasih telah bergabung. Akun Anda saat ini sedang dalam proses verifikasi atau pemantauan. Silakan menunggu informasi lebih lanjut.
                            </p>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
                                <div className="bg-white/5 p-4 rounded-xl border border-white/5 flex flex-col items-center">
                                    <Clock className="w-8 h-8 text-amber-500 mb-2" />
                                    <h3 className="text-white font-medium">Status Akun</h3>
                                    <p className="text-sm text-gray-500">Menunggu Verifikasi</p>
                                </div>
                                <div className="bg-white/5 p-4 rounded-xl border border-white/5 flex flex-col items-center">
                                    <CheckCircle2 className="w-8 h-8 text-green-500 mb-2" />
                                    <h3 className="text-white font-medium">Data Diri</h3>
                                    <p className="text-sm text-gray-500">Lengkap</p>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 md:gap-4 bg-transparent border-transparent rounded-xl md:rounded-2xl p-3 md:p-6 text-foreground shadow-sm relative overflow-hidden">
                            <div className="relative z-10 space-y-1">
                                ...
                                <p className="text-sm md:text-base text-gray-500 dark:text-zinc-400">
                                    Berikut adalah ringkasan pendaftaran calon anggota hari ini, {new Date().toLocaleDateString('id-ID', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}.
                                </p>
                            </div>
                            ...
                            <Card className="relative overflow-hidden border-none shadow-[0_20px_50px_rgba(0,0,0,0.3)] group bg-[#262626] text-[#FEFCF8] transition-all duration-300 hover:shadow-[0_20px_60px_rgba(0,0,0,0.4)] hover:-translate-y-1">
                                <CardContent className="p-6">
                                    ...
                                    <div className="space-y-1 mb-4">
                                        <h3 className="text-sm font-medium text-blue-400">Total Pendaftar</h3>
                                        <div className="text-4xl font-bold text-[#FEFCF8]">{totalUsers}</div>
                                    </div>
                                    <div className="flex items-center text-xs text-gray-400">
                                        <span>Total akumulasi calon anggota KOMCAD</span>
                                    </div>
                                </CardContent>
                            </Card>
                                ...
                                <CardContent className="p-6">
                                    <div className="flex justify-between items-start mb-4">
                                        <div className="p-3 bg-[#659800]/10 rounded-xl transition-colors group-hover:bg-[#659800]/20">
                                            <Shield className="h-6 w-6 text-[#659800]" />
                                        </div>
                                    </div>
                                    <div className="space-y-1 mb-4">
                                        <h3 className="text-sm font-medium text-[#659800]">Matra AD</h3>
                                        <div className="text-4xl font-bold text-[#FEFCF8]">{matraCounts?.AD || 0}</div>
                                    </div>
                                    <div className="flex items-center text-xs text-gray-400">
                                        <span>Angkatan Darat</span>
                                    </div>
                                </CardContent>
                            </Card>
                                ...
                                <CardContent className="p-6">
                                    <div className="flex justify-between items-start mb-4">
                                        <div className="p-3 bg-[#d04438]/10 rounded-xl transition-colors group-hover:bg-[#d04438]/20">
                                            <Anchor className="h-6 w-6 text-[#d04438]" />
                                        </div>
                                    </div>
                                    <div className="space-y-1 mb-4">
                                        <h3 className="text-sm font-medium text-[#d04438]">Matra AL</h3>
                                        <div className="text-4xl font-bold text-[#FEFCF8]">{matraCounts?.AL || 0}</div>
                                    </div>
                                    <div className="flex items-center text-xs text-gray-400">
                                        <span>Angkatan Laut</span>
                                    </div>
                                </CardContent>
                            </Card>
                                ...
                                <CardContent className="p-6">
                                    <div className="flex justify-between items-start mb-4">
                                        <div className="p-3 bg-[#AC0021]/10 rounded-xl transition-colors group-hover:bg-[#AC0021]/20">
                                            <Plane className="h-6 w-6 text-[#AC0021]" />
                                        </div>
                                    </div>
                                    <div className="space-y-1 mb-4">
                                        <h3 className="text-sm font-medium text-[#AC0021]">Matra AU</h3>
                                        <div className="text-4xl font-bold text-[#FEFCF8]">{matraCounts?.AU || 0}</div>
                                    </div>
                                    <div className="flex items-center text-xs text-gray-400">
                                        <span>Angkatan Udara</span>
                                    </div>
                                </CardContent>
                            </Card>
                        </div>
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <CardTitle className="flex items-center gap-2 text-[#FEFCF8]">
                                                ...
                                                Distribusi Calon Anggota
                                            </CardTitle>
                                            <CardDescription className="text-[#B0B0B0]">
                                                Peta sebaran {totalUsers.toLocaleString()} calon anggota di seluruh Indonesia
                                            </CardDescription>
                                        </div>
                                    </div>
                                </CardHeader>
            ...
            <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
                <DialogContent className="sm:max-w-[600px] bg-[#1e1e1e] border-zinc-800 text-zinc-100 max-h-[80vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle>Detail Data Pendaftar</DialogTitle>
                        <DialogDescription className="text-zinc-400">
                            Data pendaftar provinsi: {selectedProvince}
                        </DialogDescription>
                    </DialogHeader>
                    <div className="py-4">
                        {selectedProvinceData.length > 0 ? (
                            <Table>
                                <TableHeader>
                                    <TableRow className="border-zinc-800 hover:bg-zinc-800/50">
                                        <TableHead className="text-zinc-400">Pekerjaan</TableHead>
                                        <TableHead className="text-center text-zinc-400">Laki-laki</TableHead>
                                        <TableHead className="text-center text-zinc-400">Perempuan</TableHead>
                                    </TableRow>
                                </TableHeader>
                                <TableBody>
                                    {selectedProvinceData.map((row, index) => (
                                        <TableRow key={index} className="border-zinc-800 hover:bg-zinc-800/50">
                                            ...
                                            <TableCell className="text-center text-zinc-300">{row.L}</TableCell>
                                            <TableCell className="text-center text-zinc-300">{row.P}</TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
                                <TableFooter className="bg-zinc-800/50 border-zinc-800">
                                    <TableRow>
                                        <TableCell className="font-bold text-zinc-200">TOTAL</TableCell>
                                        <TableCell className="text-center font-bold text-zinc-200">
                                            {selectedProvinceData.reduce((acc, curr) => acc + curr.L, 0)}
                                        </TableCell>
                                        <TableCell className="text-center font-bold text-zinc-200">
                                            {selectedProvinceData.reduce((acc, curr) => acc + curr.P, 0)}
                                        </TableCell>
                                    </TableRow>
                                </TableFooter>
                            </Table>
                        ) : (
                            <p className="text-center text-zinc-500 py-4">Tidak ada data detail tersedia.</p>
                        )}
                    </div>
                </DialogContent>
            </Dialog>
