...
export default function IndonesiaMap({ data, totalUsers, onProvinceClick, enableZoom = false }: Props) {
    ...
    return (
        <div className="w-full h-[400px] rounded-lg overflow-hidden border border-border bg-[#1F2937]">
            <MapContainer
                ...
                zoom={4.9}
                minZoom={enableZoom ? 3 : 4.9}
                maxZoom={enableZoom ? 10 : 4.9}
                maxBounds={indonesiaBounds}
                maxBoundsViscosity={1.0}
                style={{ height: '100%', width: '100%', backgroundColor: '#1F2937' }}
                className="z-0"
                scrollWheelZoom={enableZoom}
                zoomControl={false} // Keep false for cleanliness or enable? User said "Zoom in zoom out", usually implies gestures or buttons. Let's enable touch/double click but maybe keep buttons hidden if they take space, or just use enableZoom. Let's set zoomControl={enableZoom}
                dragging={true}
